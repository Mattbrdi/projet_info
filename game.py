...
class Game: 
    ...
    def run(self, carte_filename):
        """Run the game"""
        self.level = 0
        while self.level < len(carte_filename): 
            carte = Map(carte_filename[self.level])
            ...
            blue_portal = Portail(screen, carte_filename[self.level], 'blue')
            orange_portal = Portail(screen, carte_filename[self.level], 'orange')
            blue_portal.is_selected = True # Par défaut, le portail bleu est sélectionné

            ##Fin du niveau
            coord_end = perso.end_coordonate(carte)
            running = True
            while running:
                ...
                # Fin du niveau
                if perso.next_level(coord_end, carte):
                    self.level+=1
                    running = False                
        credits = Credits()
        credits.run()